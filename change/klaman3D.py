...
if __name__ == "__main__":
    ...
    # 创建状态变量的真实值的矩阵 状态变量1：速度 状态变量2：位置 状态变量3：高度
    speed_true = []
    position_true = []
    height_true = []
    ...
    for i in range(30):
        ...
        Q_sigma = np.array([[math.sqrt(Q[0, 0]), Q[0, 1], Q[0, 2]], [Q[1, 0], math.sqrt(Q[1, 1]), Q[1, 2]], [Q[2, 0], Q[2, 1], math.sqrt(Q[2, 2])]])
        w = np.array([[gaussian_distribution_generator(Q_sigma[0, 0])], [gaussian_distribution_generator(Q_sigma[1, 1])], [gaussian_distribution_generator(Q_sigma[2, 2])]])

        # 真实值X_true 得到当前时刻的状态；之前我一直在想它怎么完成从Xk-1到Xk的更新，实际上在代码里面直接做迭代就行了，这里是不涉及数组下标的！！！
        # dot函数用于矩阵乘法，对于二维数组，它计算的是矩阵积
        X_true = np.dot(A, X_true) + w

        # 速度的真实值是speed_true 使用append函数可以把每一次循环中产生的拼接在一起，形成一个新的数组speed_true
        speed_true.append(X_true[0, 0])
        position_true.append(X_true[1, 0])
        height_true.append(X_true[2, 0])

        # --------------------生成观测值-----------------------------
        # 生成过程噪声
        R_sigma = np.array([[math.sqrt(R[0, 0]), R[0, 1], R[0, 2]], [R[1, 0], math.sqrt(R[1, 1]), R[1, 2]], [R[2, 0], R[2, 1], math.sqrt(R[2, 2])]])
        v = np.array([[gaussian_distribution_generator(R_sigma[0, 0])], [gaussian_distribution_generator(R_sigma[1, 1])], [gaussian_distribution_generator(R_sigma[2, 2])]])

        # 生成观测值Z_measure 取H为单位阵
        Z_measure = np.dot(H, X_true) + v
        speed_measure.append(Z_measure[0, 0])
        position_measure.append(Z_measure[1, 0])
        height_measure.append(Z_measure[2, 0])
        ...
        # 把k时刻先验预测值赋给三个状态分量的先验预测值 speed_prior_est = X_prior[0,0];position_prior_est=X_prior[1,0];height_prior_est=X_prior[2,0]
        # 再利用append函数把每次循环迭代后的分量值拼接成一个完整的数组
        speed_prior_est.append(X_prior[0, 0])
        position_prior_est.append(X_prior[1, 0])
        height_prior_est.append(X_prior[2, 0])
        ...
        k2 = np.dot(H, k1) + R
        # k3 = np.dot(np.dot(H, P_prior), H.T) + R k2和k3都是两种写法，可以互相替代
        K = np.dot(k1, np.linalg.inv(k2))

        # step4:利用卡尔曼增益K 进行校正更新状态，得到k时刻的后验估计状态X_posterior_new
        X_posterior_1 = Z_measure - np.dot(H, X_prior)
        X_posterior = X_prior + np.dot(K, X_posterior_1)
        # 把k时刻后验预测值赋给三个状态分量的后验预测值 speed_posterior_est = X_posterior[0,0];position_posterior_est = X_posterior[1,0];height_posterior_est = X_posterior[2,0]
        speed_posterior_est.append(X_posterior[0, 0])
        position_posterior_est.append(X_posterior[1, 0])
        height_posterior_est.append(X_posterior[2, 0])

        # step5:更新k时刻的误差的协方差矩阵 为估计k+1时刻的最优值做准备
        P_posterior_1 = np.eye(3) - np.dot(K, H)
        P_posterior = np.dot(P_posterior_1, P_prior)

    # ---------------------再从step5回到step1 其实全程只要搞清先验后验 k的自增是隐藏在循环的过程中的 然后用分量speed和position的append去记录每一次循环的结果-----------------------------

    # 可视化显示画出速度比较和位置比较以及高度比较
    if True:
        # 画出1行3列的多子图
        ...
        # 高度
        axs[2].plot(height_true, "-", color="blue", label="高度真实值", linewidth="1")
        axs[2].plot(height_measure, "-", color="grey", label="高度测量值", linewidth="1")
        axs[2].plot(height_prior_est, "-", color="green", label="高度先验估计值", linewidth="1")
        axs[2].plot(height_posterior_est, "-", color="red", label="高度后验估计值", linewidth="1")
