...
#import pyzed.sl as sl  # 新增ZED SDK依赖
# Parameters
...
class LoadSVO:
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        if err != sl.ERROR_CODE.SUCCESS:
            raise RuntimeError(f"Failed to open SVO file: {path}")

        self.runtime = sl.RuntimeParameters()
        self.img_size = img_size
        self.stride = stride
        self.auto = auto
        self.nf = self.zed.get_svo_number_of_frames()
        self.current_frame = 0

        # 初始化深度图
        self.depth_map = sl.Mat()
        self.depth_info = sl.Mat()
    ...
    def __next__(self):
        ...
        if self.zed.grab(self.runtime) == sl.ERROR_CODE.SUCCESS:
            ...
            img0 = self.image.get_data()
            depth_data = self.depth_map.get_data()  # 深度数据矩阵

            self.current_frame += 1
            return img0, depth_data, self.current_frame, ''
        else:
            raise StopIteration
